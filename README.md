...
Real project photos are organized **by job**, one folder per completed project:

```
public/images/projects/<service>/<job-slug>/photo-1.jpg, photo-2.jpg, …
```

`photo-1.jpg` is the cover. Each job is registered in the `JOBS` manifest in
[`lib/site.ts`](lib/site.ts) (slug, service, title, location, photo count) — the
homepage rail and the by-job galleries on `/projects` both read from it.

**To add a new job's photos:** create `public/images/projects/<service>/<slug>/`,
drop the photos in named `photo-1.jpg …`, then add one `JOBS` entry. That's it.

Trades not yet photographed (kitchens, decks, basements, additions) still use the
on-brand SVG placeholders in `public/images/` generated by
